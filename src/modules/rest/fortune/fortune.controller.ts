...
export class FortuneController {
    ...
    async getFortune(
        ...
        if (!updatedBalance) {
            const msg = `Insufficient gems to get midnight wish.  Player with tgId: ${tgId} has ${balance.gems} gems.`
            this.logger.error(msg)
            throw new BadRequestException(msg)
        }

        if (wish) {
            this.logger.debug(`Success getting midnight for tgId: ${req.currentUser?.tgId}`)
        }

        return reply.type('application/json').send({wish, balance: updatedBalance, actionInstance  });
    }
